...
export function step(RADIUS, sceneEntities, obstacleEntities, world, WORLDUNIT, mode, field, opens=[], customParam) {
    ...
    function findPotentialLeaders(item, agents, lookHeader=false){
        let candidates = [];
    ...
    function realisticFollowing(item, agents){
        ...
        if(followingConditions){
            ...
            acceleration = 0.4 * delta_v_x / Math.pow(minimum_delta_p_x, -0.3);
        ...
        //
            ...
            // console.log("Trigger");
            acceleration = 0.01 * WALKINGDIR * (item.v_pref - Math.abs(item.prev_vx)) / timestep
        ...
        // used to stop agent flow so should be like a static object
        if(item.obstacle){
            acceleration = 0;
        }
    ...
    // however, this parameter does not be used in paper "Realistic following behaviors for crowd simulation"
    // it is to make the follower always follow the front closest agent no matter the distance
    // const EPSILON_p_x = 1.5 * (RADIUS / 0.3);
    const EPSILON_p_x = 3.9 * (RADIUS / 0.3);
    ...
    // determine which agents at the head of flow
    // sceneEntities.forEach(function (item){
    //     let potentialLeaders = findPotentialLeaders(item, sceneEntities, true);
    //     let followingConditions = potentialLeaders.length > 0;
    //     item.header = !followingConditions;
    // });

    // default action if no constrain exists
    agentVelocityPlannerV5();
    // get obstacles
    let obs = filterObstacles(sceneEntities);
    ...
    sceneEntities.forEach(function (item) {
        ...
        if(customParam.stop){
            ...
            obs.forEach(function (item){
                item.x = item.sx - customParam.globalCounter * 40;
                item.px = item.sx - customParam.globalCounter * 40;
            });
