...
let plane, dist, angle, sceneEntities = [];
...
function init() {
    ...
    glbLoader.load('resources/pinetree.glb', function(gltf) {
        tree = gltf.scene;
    ...
    glbLoader.load('resources/plane.glb', function(gltf) {
        plane = gltf.scene;
...
function generateTrees(expr) {
    let treeCtr = 0,
        totalTreeNum = 500,
        zPos, xPos, spacing = 22;
    treePositions = [];
    switch (expr) {
        case 'Random':
            while (treeCtr < totalTreeNum) {
                zPos = -world.z / 2 + Math.floor(treeCtr / spacing) * world.z / spacing;
...
function animate() {
    ...
    for (var i = 0; i < sceneEntities.length; i++) {
        dist = Math.sqrt(sceneEntities[i].position.z * sceneEntities[i].position.z +
            sceneEntities[i].position.x * sceneEntities[i].position.x);
        angle = Math.atan2(sceneEntities[i].position.z, sceneEntities[i].position.x);
        angle += 0.005;
